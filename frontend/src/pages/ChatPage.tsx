          ...
          {messages.map(message => (
            ...
            >
              <div className={`flex max-w-[80%] ${message.sender === 'user' ? 'flex-row-reverse' : ''}`}>
                <div className={`flex-shrink-0 h-8 w-8 rounded-full flex items-center justify-center ${message.sender === 'user' ? 'bg-blue-100 ml-2' : 'bg-gray-100 mr-2'
                  }`}>
                  {message.sender === 'user'
                ...
                <div>
                  <div className={`rounded-lg px-4 py-2 mb-1 ${message.sender === 'user'
                      ? 'bg-blue-600 text-white'
                      : 'bg-gray-100 text-gray-800'
                    }`}>
                    <p className="text-sm whitespace-pre-wrap">{message.text}</p>
                  </div>
                  <p className={`text-xs ${message.sender === 'user' ? 'text-right text-gray-500' : 'text-gray-500'
                    }`}>
        ...
        <div className="border-t border-gray-200 p-4">
          <div className="flex items-end space-x-2">
            ...
            <button
              ...
              className={`p-2 rounded-full ${inputValue.trim() === '' || isLoading
